...
- порядок гарантируется внутри партиции (про партиции в [Топиках и партициях](../../../01-02-topics-and-partitions/i18n/ru/README.md));
...
- compliance-данные (годами) уходят в S3, а не в Kafka - это будет разъяснено в [Offsets и Retention](../../../01-04-offsets-and-retention/i18n/ru/README.md).
...
Брокер и контроллер живут на сервере (в стенде - в docker compose). Продьюсер и консьюмер - в твоём Go-приложении. Один процесс может быть и продьюсером, и консьюмером одновременно (классический паттерн для consume-process-produce - см. [Consume-Process-Produce](../../../../04-reliability/04-02-consume-process-produce/i18n/ru/README.md)).
...
Min ISR = 2, default replication factor = 3. Это значит: данные есть на трёх нодах, и при записи нужно подтверждение от двух. Если упадёт одна - не заметишь. Упадут две - продьюсер с `acks=all` начнёт получать `NotEnoughReplicas`. Подробности про эти настройки в [Acks & Durability](../../../../02-producer/02-02-acks-and-durability/i18n/ru/README.md) и в [Transactions & EOS](../../../../04-reliability/04-01-transactions-and-eos/i18n/ru/README.md).
...
Дальше в курсе CLI почти не зовём - всё через franz-go и kadm. Сюда вернёмся в [Consumer Groups & Rebalance](../../../../03-consumer/03-01-groups-and-rebalance/i18n/ru/README.md) и в [Transactions & EOS](../../../../04-reliability/04-01-transactions-and-eos/i18n/ru/README.md), когда понадобится знать, кто сейчас controller, чтобы понять последствия его перевыборов.
